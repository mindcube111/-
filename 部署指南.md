# Cloudflare Pages 部署指南

## 📋 部署前准备清单

### 1. 必需工具
- [ ] Node.js 18+ 已安装
- [ ] Wrangler CLI 已安装：`npm install -g wrangler`
- [ ] Cloudflare 账号已注册
- [ ] 域名已接入 Cloudflare（NS 已切换）

### 2. 登录 Cloudflare
```bash
wrangler login
```
这会打开浏览器，完成登录授权。

---

## 🚀 部署步骤

### 步骤 1: 创建 KV 命名空间

1. **通过 Dashboard 创建（推荐）**
   - 访问：https://dash.cloudflare.com
   - 进入：Workers & Pages → KV → Create a namespace
   - 命名：`INVITE_CODES`
   - 记录下显示的 **Namespace ID**

2. **或通过命令行创建**
   ```bash
   wrangler kv:namespace create "INVITE_CODES"
   ```
   记录返回的 `id` 值。

### 步骤 2: 准备邀请码文件

1. 在项目根目录创建 `invites.txt`（**不要提交到 Git**）
   ```
   ABC123
   DEF456
   GHI789
   ...
   ```
   每行一个邀请码。

2. 生成随机盐值（用于哈希邀请码）
   ```bash
   # Windows PowerShell
   [Convert]::ToBase64String([System.Security.Cryptography.RandomNumberGenerator]::GetBytes(32))
   
   # 或使用在线工具生成一个随机字符串（至少 32 字符）
   ```
   记录这个盐值，稍后需要配置到环境变量。

### 步骤 3: 生成 KV 导入文件

```bash
# 设置环境变量（Windows PowerShell）
$env:INVITE_SALT="你的随机盐值"

# 生成 bulk.json
node tools/generate-bulk.js
```

这会生成 `bulk.json` 文件，包含哈希后的邀请码数据。

### 步骤 4: 导入邀请码到 KV

```bash
# 查询命名空间 ID（如果忘记了）
wrangler kv:namespace list

# 导入邀请码（替换 <NAMESPACE_ID> 为实际的 ID）
wrangler kv:bulk put --namespace-id <NAMESPACE_ID> bulk.json

# 验证导入（可选）
wrangler kv:key get --namespace-id <NAMESPACE_ID> <某个hash键>
```

### 步骤 5: 创建 Cloudflare Pages 项目

1. **通过 Dashboard 创建**
   - 访问：https://dash.cloudflare.com
   - 进入：Workers & Pages → Create application → Pages → Connect to Git
   - 连接你的 Git 仓库（GitHub/GitLab/Bitbucket）
   - 项目名称：`stranger-things-test`
   - 构建命令：留空（静态站点）
   - 构建输出目录：`/`（根目录）

2. **或通过命令行创建**
   ```bash
   wrangler pages project create stranger-things-test
   ```

### 步骤 6: 配置环境变量和 KV 绑定

在 Cloudflare Dashboard 中：

1. **进入项目设置**
   - Pages → stranger-things-test → Settings

2. **配置环境变量**
   - Environment variables → Add variable
   - 添加以下变量：
     - `ADMIN_PASSWORD`: 你的管理员密码
     - `INVITE_SALT`: 步骤 2 生成的随机盐值

3. **绑定 KV 命名空间**
   - KV namespaces → Add binding
   - Variable name: `INVITE_CODES`
   - KV namespace: 选择之前创建的 `INVITE_CODES`

### 步骤 7: 部署代码

#### 方式 A: 自动部署（推荐）
- 推送代码到 Git 主分支
- Cloudflare Pages 会自动检测并部署

#### 方式 B: 手动部署
```bash
# 确保在项目根目录
wrangler pages deploy . --project-name=stranger-things-test
```

### 步骤 8: 绑定自定义域名（可选）

1. 在 Pages 项目设置中
2. Custom domains → Set up a custom domain
3. 输入你的域名
4. 按照提示配置 DNS 记录（通常是 CNAME）

---

## ✅ 部署后验证

### 1. 检查网站是否可访问
访问你的 Pages URL：`https://stranger-things-test.pages.dev`

### 2. 测试邀请码 API
```bash
# 使用 curl 或 Postman 测试
curl -X POST https://你的域名/api/verify-invite \
  -H "Content-Type: application/json" \
  -d '{"code":"ABC123"}'

# 预期响应（首次使用）：
# {"ok":true}

# 再次使用相同邀请码：
# {"ok":false,"message":"used"}
```

### 3. 测试管理员登录 API
```bash
curl -X POST https://你的域名/api/admin-login \
  -H "Content-Type: application/json" \
  -d '{"password":"你的管理员密码"}'

# 预期响应：
# {"ok":true}
```

### 4. 检查前端功能
- [ ] 首页正常加载
- [ ] 邀请码验证功能正常
- [ ] 测试页面正常
- [ ] 结果页面正常
- [ ] 管理员页面正常

---

## 🔒 安全建议

### 1. 设置速率限制
在 Cloudflare Dashboard：
- Security → WAF → Rate limiting rules
- 为 `/api/verify-invite` 设置限流（如每 IP 每分钟 60 次）

### 2. 保护敏感文件
确保以下文件在 `.gitignore` 中：
```
invites.txt
bulk.json
.env
*.log
```

### 3. 定期更新
- 定期更换 `INVITE_SALT`
- 定期更新管理员密码
- 监控 API 使用情况

---

## 🛠️ 常见问题

### Q: 部署后 Functions 不工作？
A: 检查：
- 环境变量是否正确配置
- KV 命名空间是否正确绑定
- Functions 文件路径是否正确（`functions/api/*.js`）

### Q: 邀请码验证失败？
A: 检查：
- `INVITE_SALT` 是否与生成 bulk.json 时使用的相同
- KV 中是否已导入邀请码
- 邀请码是否已被使用

### Q: 如何添加新邀请码？
A: 
1. 更新 `invites.txt`
2. 重新运行 `node tools/generate-bulk.js`
3. 重新导入：`wrangler kv:bulk put --namespace-id <ID> bulk.json`

### Q: 如何重置已使用的邀请码？
A: 重新导入相同的 hash 键，但将 `used` 设为 `false`

---

## 📝 快速命令参考

```bash
# 登录
wrangler login

# 查看 KV 命名空间
wrangler kv:namespace list

# 生成邀请码文件
$env:INVITE_SALT="你的盐值"
node tools/generate-bulk.js

# 导入邀请码
wrangler kv:bulk put --namespace-id <ID> bulk.json

# 部署
wrangler pages deploy . --project-name=stranger-things-test

# 查看部署日志
wrangler pages deployment list --project-name=stranger-things-test
```

---

## 📞 需要帮助？

如果遇到问题，检查：
1. Cloudflare Dashboard 中的错误日志
2. Wrangler 命令的输出信息
3. 浏览器控制台的错误信息

