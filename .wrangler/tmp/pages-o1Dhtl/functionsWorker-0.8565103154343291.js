...
var onRequestPost7 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const MAX_USE_COUNT2 = 3;
  const updatedRecord = {
    ...record,
    lastUsedAt: lastUsedAt || (/* @__PURE__ */ new Date()).toISOString()
  };
  if (usedCount !== void 0) {
    const newUsedCount = Math.min(usedCount, MAX_USE_COUNT2);
    updatedRecord.usedCount = newUsedCount;
    updatedRecord.used = newUsedCount >= MAX_USE_COUNT2;
    if (updatedRecord.usedCount >= MAX_USE_COUNT2) {
...
var onRequestPost8 = /* @__PURE__ */ __name(async ({ request, env }) => {
  ...
  const hash = await sha256Hex3(env.INVITE_SALT + code);
  console.log("\u9A8C\u8BC1\u9080\u8BF7\u7801:", {
    code,
    saltLength: env.INVITE_SALT ? env.INVITE_SALT.length : 0,
    hash: hash.substring(0, 16) + "...",
  ...
  await env.INVITE_CODES.put(hash, JSON.stringify(updatedRecord));
  return Response.json({ ok: true, usedCount, remaining: MAX_USE_COUNT - usedCount });
}, "onRequestPost");

// ../.wrangler/tmp/pages-o1Dhtl/functionsRoutes-0.6717762789152228.mjs
var routes = [
  {
  ...
  {
    routePath: "/api/update-invite-usage",
    mountPath: "/api",
    method: "POST",
    middlewares: [],
